...
for indir,nm in zip(indirs,names):
    ...
    #except:
    #   pass
    if not model or nm not in kmer_dict: #xcept IOError:
       num_reads = 0 
       kmer_dict[nm] = {}
       loc = None
       version = None
...
colpal = zip(['#56d6e1','#2b2381','#db1818'],['#b5e5ea','#ada8e7','#d89494'])
for col,nm in zip(colpal,sorted(names)):
    if len(names) < 4:
...
plt.xlabel('k-mer')
plt.savefig(name+'_current_dist.eps',dpi=500,bbox_inches='tight')
plt.savefig(name+'_current_dist.png',dpi=500,bbox_inches='tight')
